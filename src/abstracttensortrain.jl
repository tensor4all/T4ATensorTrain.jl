...
Implementations: [`TensorTrain`](@ref)
"""
abstract type AbstractTensorTrain{V} <: Function end
...
Returns the bond dimensions along the links between neighboring tensors.

See also: [`rank`](@ref)
"""
function linkdims(tt::AbstractTensorTrain{V})::Vector{Int} where {V}
    # For a TT with cores (r_{n-1}, d_n, r_n), the link dimensions are r_1..r_{L-1}
    return [Base.size(T, 1) for T in tt[2:end]]
end
...
"""
function linkdim(tt::AbstractTensorTrain{V}, i::Int)::Int where {V}
    return Base.size(sitetensor(tt, i + 1), 1)
end
...
"""
function sitedims(tt::AbstractTensorTrain{V})::Vector{Vector{Int}} where {V}
    return [collect(Base.size(T)[2:end-1]) for T in tt]
end
...
function sitedim(tt::AbstractTensorTrain{V}, i::Int)::Vector{Int} where {V}
    return collect(Base.size(sitetensor(tt, i))[2:end-1])
end
...
function rank(tt::AbstractTensorTrain{V}) where {V}
    lds = linkdims(tt)
    return isempty(lds) ? 1 : maximum(lds)
end
...
"""
function length(tt::AbstractTensorTrain{V}) where {V}
    return Base.length(sitetensors(tt))
end
...
function evaluate(
    ...
    indexset::Union{
        ...
        AbstractVector{<:NTuple{N,Int} where N},
        NTuple{M,AbstractVector{Int}} where M,
        NTuple{M,NTuple{N,Int} where N} where M}
) where {V}
    if length(indexset) != length(tt)
        throw(ArgumentError("To evaluate a tt of length $(length(tt)), you have to provide $(length(tt)) indices, but there were $(length(indexset))."))
    end
    for (n, (T, i)) in enumerate(zip(tt, indexset))
        Base.length(Base.size(T)) == Base.length(i) + 2 || throw(ArgumentError("The index set $(i) at position $n does not have the correct length for the tensor of size $(Base.size(T))."))
    end
    return only(prod(T[:, i..., :] for (T, i) in zip(tt, indexset)))
end
...
function sum(tt::AbstractTensorTrain{V}) where {V}
    v = transpose(Base.sum(tt[1], dims=(1, 2))[1, 1, :])
    for T in tt[2:end]
        v *= Base.sum(T, dims=2)[:, 1, :]
    end
    return only(v)
end
...
function _addtttensor(
    ...
    nd = ndims(A)
    offset1 = lefttensor ? 0 : Base.size(A, 1)
    offset3 = righttensor ? 0 : Base.size(A, nd)
    localindices = fill(Colon(), nd - 2)
    C = zeros(V, offset1 + Base.size(B, 1), Base.size(A)[2:nd-1]..., offset3 + Base.size(B, nd))
    C[1:Base.size(A, 1), localindices..., 1:Base.size(A, nd)] = factorA * A
...
function LA.norm2(tt::AbstractTensorTrain{V})::Float64 where {V}
    function _f(n)::Matrix{V}
        t = sitetensor(tt, n)
        t3 = reshape(t, Base.size(t)[1], :, Base.size(t)[end])
        # (lc, s, rc) * (l, s, r) => (lc, rc, l, r)
        tct = _contract(conj.(t3), t3, (2,), (2,))
        tct = permutedims(tct, (1, 3, 2, 4))
        return reshape(tct, Base.size(tct, 1) * Base.size(tct, 2), Base.size(tct, 3) * Base.size(tct, 4))
    end
