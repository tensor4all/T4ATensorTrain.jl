...
struct TTCache{ValueType} <: BatchEvaluator{ValueType}
    ...
    function TTCache{ValueType}(sitetensors::AbstractVector{<:AbstractArray{ValueType}}, sitedims) where {ValueType}
        Base.length(sitetensors) == Base.length(sitedims) || throw(ArgumentError("The number of site tensors and site dimensions must be the same."))
        for n in 1:Base.length(sitetensors)
            prod(sitedims[n]) == prod(Base.size(sitetensors[n])[2:end-1]) || error("Site dimensions do not match the site tensor dimensions at $n.")
        end
        new{ValueType}(
            [reshape(x, Base.size(x, 1), :, Base.size(x)[end]) for x in sitetensors],
    ...
    function TTCache{ValueType}(sitetensors::AbstractVector{<:AbstractArray{ValueType}}) where {ValueType}
        return TTCache{ValueType}(sitetensors, [collect(Base.size(x)[2:end-1]) for x in sitetensors])
    end
end
...
Base.length(obj::TTCache) = Base.length(obj.sitetensors)
...
function sitetensor(tt::TTCache{V}, i::Integer) where {V}
    ...
    return reshape(
        sitetensor,
        Base.size(sitetensor)[1], tt.sitedims[i]..., Base.size(sitetensor)[end]
    )
end
...
function evaluateright(
    ...
    if Base.length(indexset) == 0
        return V[1]
    elseif Base.length(indexset) == 1
        return vec(tt.sitetensors[end][:, indexset[1], :])
    end
    ell = length(tt) - Base.length(indexset) + 1
...
function evaluate(
    ...
    if length(tt) != Base.length(indexset)
        throw(ArgumentError("To evaluate a tensor train of length $(length(tt)), need $(length(tt)) index values, but got $(Base.length(indexset))."))
    end
    if usecache
        return _dot(
            evaluateleft(tt, view(indexset, 1:midpoint)),
            evaluateright(tt, view(indexset, midpoint+1:Base.length(indexset))))
    else
...
function batchevaluate(tt::TTCache{V},
    ...
    if Base.length(leftindexset) * Base.length(rightindexset) == 0
        return Array{V,M + 2}(undef, ntuple(d -> 0, M + 2)...)
    end
    N = length(tt)
    nleft = Base.length(leftindexset[1])
    nright = Base.length(rightindexset[1])
    nleftindexset = Base.length(leftindexset)
    nrightindexset = Base.length(rightindexset)
    ...
    if projector === nothing
        projector = [fill(0, Base.length(s)) for s in sitedims(tt)[nleft+1:(N-nright)]]
    end
    if Base.length(projector) != M
        error("Invalid length of projector: $(projector), correct length should be M=$M")
    end
    for n in s_:e_
        Base.length(projector[n-s_+1]) == Base.length(tt.sitedims[n]) || error("Invalid projector at $n: $(projector[n - s_ + 1]), the length must be $(Base.length(tt.sitedims[n]))")
    ...
    for n in nleft+1:(N-nright)
        ...
        T_ = begin
            ...
            reshape(s, Base.size(s)[1], :, Base.size(s)[end])
        end
        localdim[n-nleft] = Base.size(T_, 2)
        bonddim_ = Base.size(T_, 1)
        lenv = reshape(lenv, :, bonddim_) * reshape(T_, bonddim_, :)
    end

    # (nleftindexset, d...d * D) x (D, nrightindexset)
    bonddim_ = Base.size(renv, 1)
