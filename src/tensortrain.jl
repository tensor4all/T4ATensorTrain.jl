...
mutable struct TensorTrain{ValueType,N} <: AbstractTensorTrain{ValueType}
    ...
    function TensorTrain{ValueType,N}(sitetensors::AbstractVector{<:AbstractArray{ValueType,N}}) where {ValueType,N}
        for i in 1:Base.length(sitetensors)-1
...
function TensorTrain{V2,N}(tt::AbstractTensorTrain{V}, localdims)::TensorTrain{V2,N} where {V,V2,N}
    for d in localdims
        Base.length(d) == N - 2 || error("Each element of localdims be a list of N-2 integers.")
    end
    ...
    return TensorTrain{V2,N}(
        [reshape(Array{V2}(t), Base.size(t, 1), localdims[n]..., Base.size(t)[end]) for (n, t) in enumerate(sitetensors(tt))])
end
...
function _factorize(
    ...
    elseif method === :SVD
        ...
        err = [Base.sum(factorization.S[n+1:end] .^ 2) for n in 1:Base.length(factorization.S)]
        normalized_err = err ./ Base.sum(factorization.S .^ 2)

        trunci = min(
            replacenothing(findfirst(<(abstol^2), err), Base.length(err)),
...
function compress!(
    ...
    for ell in 1:length(tt)-1
        shapel = Base.size(tt.sitetensors[ell])
        ...
        shaper = Base.size(tt.sitetensors[ell+1])
    ...
    for ell in length(tt):-1:2
        shaper = Base.size(tt.sitetensors[ell])
        left, right, newbonddim = _factorize(
        ...
        shapel = Base.size(tt.sitetensors[ell-1])
...
function TensorTrainFit{ValueType}(indexsets, values, tt) where {ValueType}
    offsets = [0]
    for n in 1:length(tt)
        push!(offsets, offsets[end] + Base.length(tt[n]))
    end
...
function (obj::TensorTrainFit{ValueType})(x::Vector{ValueType}) where {ValueType}
    ...
    return Base.sum((abs2(_evaluate(tensors, indexset) - obj.values[i]) for (i, indexset) in enumerate(obj.indexsets)))
end
...
function fulltensor(obj::TensorTrain{T,N})::Array{T} where {T,N}
    ...
    for l in 2:length(obj)
        nextmatrix = reshape(
            obj.sitetensors[l], Base.size(obj.sitetensors[l], 1), localdims[l] * Base.size(obj.sitetensors[l])[end])
        leftdim *= localdims[l]
        result = reshape(result * nextmatrix, leftdim, Base.size(obj.sitetensors[l])[end])
    end
